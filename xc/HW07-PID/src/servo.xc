		...
		chanend in_servo_cmd_chan)
{
	...
	while(1){
		...
		for(i = 0; i < nServos; i++){
			...
			pulse_ticks = (pulse_ticks > MAX_SERVO_PULSE_US) ? MAX_SERVO_PULSE_US : pulse_ticks; // avoid overflow
			pulse_ticks = (pulse_ticks < MIN_SERVO_PULSE_US) ? MIN_SERVO_PULSE_US : pulse_ticks; // avoid underflow
