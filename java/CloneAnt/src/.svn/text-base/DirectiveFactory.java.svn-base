...
public class DirectiveFactory
{
	...
	public static Directive goHome(Ant s)
	{
		...
		//Get my colony type.
		GridType ct = s.getColony().getGrid().getType();
		
		if(s.myMap.getType() == ct)
		...
		if(s.myMap.getType() != GridType.SURFACE)
		{
			pre = goSurface(s);
			links = s.myMap.getParent().getLinksTo(ct);
		}
