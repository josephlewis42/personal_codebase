...
if not SESSION or 'login' not in SESSION.keys() or not SESSION['login']:
    ...
    if 'reload' in QUERY_DICT.keys():
        try:
            ...
            page = '''<h3>Needs:</h3> <div id="progresscontainer">
                      <h4>Current Mode:</h4>%s<hr />
                   ''' % (w[a].mind.current_need.name)

            for item in w[a].needs:
                page += item.name + ":<br />"
    ...
    #Handle page query.
        ...
        <body onLoad="javascript:Update(2000, 'body', 'needs.py?reload=true');" class="widget">
        </body>
        </html>
        ''')
